...
const int psize = 640 ; // MULTIPLO DE 32!!!  320, 640, 960, 1280, 1600, 1920, 2240, 2560
const int ngen  = 5000; //%10; 25; 10
...
// PRUEBA	
	...
	//if(it = 3 ){for(int i=0; i<nvars; i++){ d_Pop[it*nvars + i] = 0 ; 	} }
	// FIN PRUEBA
...
// Genera la poblacion inicial
	...
	for (int inum=1; inum<nvdec/two; inum++){
		...
		x = x + powf(ten,(0-inum))*X ; // x = x + powf(ten,(two-inum))*X ;
        y = y + powf(ten,(0-inum))*Y ; // y = y + powf(ten,(two-inum))*Y ;
	}
	...
	d_Sol[it] = x*x + y*y ;  // De Jong

	//float a=20, b=0.2, c=2*PI;	int n=2;  
	//d_Sol[it] = -a*expf(-b*sqrt((x*x + y*y)/n)) - expf((cos(c*x) + cos(c*y))/n) + a + expf(1) ; // Ackley
	
	//int c=10 ;  d_Sol[it] = c*two + (x*x - c*cos(two*PI*x)) + (y*y - c*cos(two*PI*y)) ; // Rastrigin's function
}
...
// random number on device
	...
	//int *h_Pop = (int *)malloc(size);	//prueba
	//cudaMemcpy(h_Pop, d_Pop, size, cudaMemcpyDeviceToHost); //prueba
	//printf("h_Pop \n");	for(int i=0;i<psize*nvars;i++) { cout<<h_Pop[i]<<endl; } //prueba

	err = cudaFree(devStates);
	if (err != cudaSuccess) {fprintf(stderr, "Failed to free device vector A (error code %s)!\n", cudaGetErrorString(err)); exit(EXIT_FAILURE); }
	
	igen = 0 ; BestSol=1.0 ; x=0.0 ; y=0.0  ;
// Bucle de las generaciones  ************************************************************
	while(BestSol > tol){
	//while(igen < ngen){
		//if(igen > ngen+1){ 	BestSol=-1 ; //exit(EXIT_FAILURE); 	}
		igen++ ;
...
// Evaluacion funcion objetivo	
		...
		//printf("h_Sol \n");	for(int i=0;i<psize;i++) { cout<<h_Sol[i]<<endl;  } //prueba
...
// Copia la población ELITE	
	...
	printf("Poblacion: %i, Generaciones: %i, Tiempo transcurrido: %f \n",psize , igen, ((double)clock() - start) / CLOCKS_PER_SEC) ;
